...
namespace Pinturería.Forms
{
    public partial class DataVentaEntryForm : Form
    {
        ...
        public string SelectedFormaPago { get; private set; }
        ...
        public DataVentaEntryForm(List<string> _ProductosSeleccionados)
        {
            InitializeComponent();

            //por cada instancia de este "DataVentaEntryForm" que se habre desde "VentasForm" le pasamos los productos que ya se registraron en la venta para que no se vuelvan a mostrar para elegir en el dropdowon de Productos
        ...
        private void LoadCombos()
        {
            using (var db = new AppDbContext())
            {
                ...
                var formas = db.FormasPago.OrderBy(f => f.Nombre).Select(f => f.Nombre).ToList();
                cmbFormaPago.Items.Clear();
                cmbFormaPago.Items.Add("Seleccionar opción");
                foreach (var f in formas) cmbFormaPago.Items.Add(f);
        ...
        private void CmbCategoria_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbCategoria.SelectedIndex <= 0)
            {
                ...
                if (cmbFormaPago.Items.Count > 0)
                    cmbFormaPago.SelectedIndex = 0; // solo si hay ítems
                return;
            }
            ...
            using (var db = new AppDbContext())
            {
                ...
                // Filtrar productos que ya se registraron en esta venta
                productos = productos
        ...
        private void CmbProducto_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            using (var db = new AppDbContext())
            {
                ...
                if (producto.Stock == 0)
                {
                    MessageBox.Show("El producto seleccionado ya no tiene más stock", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    cmbProducto.SelectedIndex = 0;
                    return;
                }

                // Poblar cantidades desde 0 hasta stock (0 será validado después)
                cmbCantidad.Enabled = true;
                cmbCantidad.Items.Clear();
                cmbCantidad.Items.Add("Seleccionar opción");
                for (int i = 0; i <= producto.Stock; i++)
        ...
        {
            // Aquí no es estrictamente necesario hacer nada más
            // Si quieres, podrías habilitar el botón Aceptar solo si FormaPago está seleccionada
            btnAceptar.Enabled = cmbFormaPago.SelectedIndex > 0;
        }
        ...
        private void BtnAceptar_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Seleccione forma de pago.", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            SelectedFormaPago = cmbFormaPago.SelectedItem.ToString();
